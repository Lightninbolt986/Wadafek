    ...
    async execute(message, args, cmd, client, Discord) {
        if (args[0] == 'lb') {
            ...
            const msg = await message.channel.send({
                embeds: [{
                    "footer": {
                        "text": "Game starts in 15 seconds. Get ready!"
                    },
                    "thumbnail": {
                        "url": "https:\/\/media.discordapp.net\/attachments\/892704135244283916\/927801529015095336\/PicsArt_01-04-11.15.34.png",
                        "proxy_url": "https:\/\/images-ext-2.discordapp.net\/external\/tycXs4P7gHyX09e4v32Lh6-p2_HmUpE5Gzyqi7xMvyk\/https\/media.discordapp.net\/attachments\/892704135244283916\/927801529015095336\/PicsArt_01-04-11.15.34.png",
            ...
            setTimeout(async function () {
                play(msg, score, profileData, drow, isPlaying, message)
            }, 15000)
...
async function play(msg, score, profileData, drow, isPlaying, message) {
    let StartArr = [ClosedDoor, ClosedDoor, ClosedDoor]
    let StartAttach = await getAttach(StartArr)
    await msg.edit({
        embeds: [],
        files: [StartAttach],
        components: [row]
    ...
    collector.on("collect", async i => {
        ...
        if (wrongDoor == DoorOpened) {
            ...
            const loseAttach = await getAttach(DoorOpenedArr)
            await msg.edit({
        ...
        score++
        const newAttach = await getAttach(DoorOpenedArr)
        await msg.edit({

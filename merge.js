    ...
    resolve(Promise.all(images)
        .then(images => {
            ...
            for (let i = 0; i < images.length; i++) {
                images[i] = Object.assign(images[i], {
                    x: xValueOfImage,
                    y: 0
                });
            ...
            if (options.text) fillTextWordWrapping(ctx, options.text, Xpadding, maxheight + Ypadding, YpaddingLines, xValueOfImage - Xpadding, options);
...
async function l(Arr) {
    let e = await mergeImages(Arr)
    var base64Data = e.replace(/^data:image\/png;base64,/, "")
    const p = Buffer.from(base64Data, 'base64')
